...
**Workaround:** Override `date.format` to `"date"` in `parser.patch.schemas` for both DTOs (see `src/codegen/schema-patches.ts`).
...
**Workaround:** In `parser.patch.schemas` (`src/codegen/schema-patches.ts`) replace both properties with `{ allOf: [property], nullable: true }` — wrapping the `$ref` in an `allOf` moves `nullable: true` to a non-`$ref` schema object, which the generator correctly converts to `z.union([zType, z.null()])` and TypeScript `Type | null`.
